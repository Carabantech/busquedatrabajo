...
export function loadState(candidateId = getActiveCandidateId()) {
  const paths = ensureCandidateDir(candidateId, candidateId);
  return loadJson(paths.statePath, defaultCandidateState());
}

export function saveState(nextState, candidateId = getActiveCandidateId()) {
  const paths = ensureCandidateDir(candidateId, candidateId);
  saveJson(paths.statePath, nextState);
}
...
function buildPortalQueries(profile, profileInputs) {
  const city = profile.location?.city || 'Mendoza';
  ...
  return { portalQueries, explicitTerms, city };
}

function rankJobs(jobs, profile, profileInputs) {
  const roleTerms = profile.target_roles?.primary || [];
  const hardSkills = [profileInputs.erpTools, profileInputs.excelLevel, 'sap', 'excel', 'erp'].filter(Boolean);
  const city = profile.location?.city || '';
  const negatives = (profileInputs.restrictions || '').split(/[,.;]/).map(item => item.trim().toLowerCase()).filter(Boolean);
  ...
  return jobs
    .map(job => {
      ...
      if (job.snippet.toLowerCase().includes(city.toLowerCase())) score += 2;
      if (job.portal === 'HiringRoom' || job.portal === 'Computrabajo') score += 2;
      if (negatives.some(term => haystack.toLowerCase().includes(term))) score -= 6;
      return { ...job, score };
    })
...
export async function searchJobsForProfile(candidateId = getActiveCandidateId()) {
  ...
  const state = loadState(candidateId);
  const profileInputs = state.profileInputs || {};
  const { portalQueries, explicitTerms } = buildPortalQueries(profile, profileInputs);
